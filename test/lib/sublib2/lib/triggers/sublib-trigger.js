...
var debugx = Devebot.require('pinbug')('devebot:test:lab:sublib2:sublibTrigger');
  ...
  self.start = function() {
    return new Promise(function(resolved, rejected) {
      var serverInstance = server.listen(configPort, configHost, function () {
        ...
        console.log('sublib2 webserver is listening at http://%s:%s', host, port);
  ...
  self.stop = function() {
    return new Promise(function(resolved, rejected) {
      server.close(function () {
        (pluginCfg && pluginCfg.verbose !== false || debugx.enabled) &&
        console.log('sublib2 webserver has been closed');
        resolved();
      });
    });
  };
